...
public class SoundEngine implements IdentifiableResourceReloadListener {
    ...
    public boolean onSoundRecieved(ClientboundSoundPacket packet) {
        ...
        var sound = Either.unwrap(event.unwrap().mapBoth(i -> i.location(), i -> i.location()));
    ...
    @Override
    public CompletableFuture<Void> reload(PreparationBarrier sync, ResourceManager sender, Executor serverExecutor, Executor clientExecutor) {
