...
public class Toggle extends Button implements IField<Boolean, Toggle> {
    ...
    protected void renderBackground(GuiGraphics context, Minecraft mc, int mouseX, int mouseY) {
        context.blitSprite(RenderType::guiTextured, TEXTURES.get(false, isHoveredOrFocused()), getX(), getY(), getWidth(), getHeight(), ARGB.white(alpha));
        int sliderX = getX() + (on ? getWidth() - 8 : 0);
        context.blitSprite(RenderType::guiTextured, TEXTURES.get(active, isHoveredOrFocused()), sliderX, getY(), 8, getHeight(), ARGB.white(alpha));
    }
