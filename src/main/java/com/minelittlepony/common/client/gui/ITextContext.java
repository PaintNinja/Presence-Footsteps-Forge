...
public interface ITextContext {
    ...
    default void drawLabel(GuiGraphics context, Component text, int x, int y, int color, double zIndex) {
        ...
        getFont().drawInBatch(text, x, y, color, true, context.pose().last().pose(), context.bufferSource(), Font.DisplayMode.SEE_THROUGH, 0, 0xF000F0);
    ...
    default void drawTextBlock(GuiGraphics context, FormattedText text, int x, int y, int maxWidth, int color) {
        for (FormattedCharSequence line : getFont().split(text, maxWidth)) {
            float left = x;
            getFont().drawInBatch(line, left, y, color, false, context.pose().last().pose(), context.bufferSource(), Font.DisplayMode.SEE_THROUGH, 0, 0xF000F0);
            context.bufferSource().endBatch();
